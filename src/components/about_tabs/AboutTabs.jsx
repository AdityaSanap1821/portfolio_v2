...
export default function AboutTabs() {
    ...
    return (
        <div className="w-full max-w-4xl mt-12 bg-white/10 rounded-xl p-6 border border-white/20 backdrop-blur-lg shadow-lg">
            {/* Tab Buttons */}
            <div className="grid grid-cols-4 mb-6 border-b border-white/20">
                {tabs.map((tab) => (
                    <button
                        key={tab}
                        onClick={() => setActiveTab(tab)}
                        className={`relative pb-2 px-3 text-lg font-semibold text-center transition-colors duration-500 ease-in-out ${activeTab === tab ? "text-white" : "text-gray-400 hover:text-white"
                            }`}
                    >
                        {tab}
                        {/* Underline effect */}
            ...
            <div
                className={`text-white text-base leading-relaxed min-h-[150px] transition-opacity duration-500 `}
            >
                {renderTab()}
            </div>
        </div>
